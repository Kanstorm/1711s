...
export async function syncChanges(oldData, newData) {
  ...
  try {
    ...
    // Bible progress
    for (const uid of Object.keys(newData.bibleProgress || {})) { const ob = oldData.bibleProgress?.[uid] || {}, nb = newData.bibleProgress[uid] || {}; for (const bn of new Set([...Object.keys(ob), ...Object.keys(nb)])) { const oc = new Set(ob[bn] || []), nc = new Set(nb[bn] || []); for (const ch of nc) { if (!oc.has(ch)) p.push(supabase.from("bible_progress").upsert({ user_id: uid, book_name: bn, chapter: ch })); } for (const ch of oc) { if (!nc.has(ch)) p.push(supabase.from("bible_progress").delete().eq("user_id", uid).eq("book_name", bn).eq("chapter", ch)); } } }
    for (const uid of Object.keys(oldData.bibleProgress || {})) { if (newData.bibleProgress?.[uid] && Object.keys(newData.bibleProgress[uid]).length === 0 && Object.keys(oldData.bibleProgress[uid]).length > 0) p.push(supabase.from("bible_progress").delete().eq("user_id", uid)); }
    // Profile fields
