...
function DirectorPage() {
  ...
  // Stats
  const testamentBooks = allBooks;
  const totalCh = testamentBooks.reduce((s, b) => s + b.ch, 0);
  const readCh = testamentBooks.reduce((s, b) => s + (bp[b.name] || []).length, 0);
  const completedBooks = testamentBooks.filter(b => (bp[b.name] || []).length >= b.ch).length;
  const overallPct = totalCh > 0 ? Math.round((readCh / totalCh) * 100) : 0;

  // Solar system layout: center sun, concentric rings for categories
  const CX = 400;
  const CY = 400;
  ...
  return (
    <div className="page-content director-page">
      ...
      {/* Testament Toggle */}
      <div style={{ display: "flex", gap: 8, marginBottom: 20 }}>
        <button className={`director-toggle ${testament === "OT" ? "active" : ""}`} onClick={() => { setTestament("OT"); setSelectedBook(null); }}>
          Old Testament
        </button>
        <button className={`director-toggle ${testament === "NT" ? "active" : ""}`} onClick={() => { setTestament("NT"); setSelectedBook(null); }}>
          New Testament
        </button>
      </div>

      {/* Stats Bar */}
      <Panel className="director-stats-bar" glow="#D4AF37">
        <div className="director-stat-item">
          <span className="director-stat-num">{readCh}</span>
          <span className="director-stat-lbl">/ {totalCh} chapters</span>
        </div>
        <div className="director-stat-divider" />
        <div className="director-stat-item">
          <span className="director-stat-num">{completedBooks}</span>
          <span className="director-stat-lbl">/ {allBooks.length} books complete</span>
        </div>
        <div className="director-stat-divider" />
        <div className="director-stat-item">
          <span className="director-stat-num" style={{ color: "#D4AF37" }}>{overallPct}%</span>
          <span className="director-stat-lbl">{testament === "OT" ? "Old" : "New"} Testament</span>
        </div>
        <div style={{ flex: 1, maxWidth: 200 }}>
          <ProgressBar value={readCh} max={totalCh} color="#D4AF37" height={6} />
        </div>
      </Panel>
      ...
      <div className="director-layout">
        ...
        <div className="solar-system-container" ref={containerRef}>
          <svg viewBox="0 0 800 800" className="solar-svg">
            ...
            {categories.map((cat, ci) => (
              <circle
                key={cat.cat}
                cx={CX} cy={CY} r={ringRadii[ci]}
                fill="none" stroke={`${cat.color}15`} strokeWidth={1}
                strokeDasharray="4 6"
              />
            ))}
            ...
            <text x={CX} y={CY - 10} textAnchor="middle" fill="#D4AF37" fontSize="22" fontWeight="700" fontFamily="Rajdhani, sans-serif">
              {overallPct}%
            </text>
            <text x={CX} y={CY + 8} textAnchor="middle" fill="#6B6152" fontSize="8" fontWeight="600" fontFamily="Rajdhani, sans-serif" letterSpacing="2">
              {testament === "OT" ? "OLD" : "NEW"}
            </text>
            <text x={CX} y={CY + 18} textAnchor="middle" fill="#6B6152" fontSize="8" fontWeight="600" fontFamily="Rajdhani, sans-serif" letterSpacing="2">
              TESTAMENT
            </text>

            {/* Category labels along rings */}
            {categories.map((cat, ci) => {
              ...
              return (
                <text
                  key={`label-${cat.cat}`}
                  x={CX} y={CY - r - 8}
                  textAnchor="middle" fill={`${cat.color}88`}
          ...
          <div className="solar-legend">
            {categories.map(cat => {
              const catBooks = cat.books;
              const catRead = catBooks.reduce((s, b) => s + (bp[b.name] || []).length, 0);
              const catTotal = catBooks.reduce((s, b) => s + b.ch, 0);
        ...
        <div className="director-detail-panel">
          ...
          {/* Quick list of all books for the testament */}
...
export default function App() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const stored = await window.storage.get("app-data-1711s-v5");
        if (stored) {
          setData(JSON.parse(stored.value));
        } else {
          const seed = getSeedData();
          setData(seed);
          await window.storage.set("app-data-1711s-v5", JSON.stringify(seed));
        }
      } catch {
        setData(getSeedData());
      }
      try {
        const user = await window.storage.get("current-user-1711s-v5");
  ...
  useEffect(() => {
    if (data) {
      window.storage.set("app-data-1711s-v5", JSON.stringify(data)).catch(() => {});
    }
  }, [data]);

  async function login(member) {
    setCurrentUser(member);
    await window.storage.set("current-user-1711s-v5", JSON.stringify(member)).catch(() => {});
  }

  function logout() {
    setCurrentUser(null);
    window.storage.delete("current-user-1711s-v5").catch(() => {});
  }
